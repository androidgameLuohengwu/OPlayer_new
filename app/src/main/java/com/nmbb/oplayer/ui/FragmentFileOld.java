...
public class FragmentFileOld extends FragmentBase implements OnItemClickListener, IMediaScannerObserver {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mParent.bindService(new Intent(mParent, MediaScannerService.class), mMediaScannerServiceConnection, Context.BIND_AUTO_CREATE);
		return v;
	}
	...
	public void onDestroy() {
		mParent.unbindService(mMediaScannerServiceConnection);
		super.onDestroy();
	}
	...
	public void onResume() {
		...
		mProgress.setVisibility(MediaScannerService.isRunning() ? View.VISIBLE : View.GONE);
	}
	...
	private void renameFile(final FileAdapter adapter, final POMedia f,
			final int position) {
		final EditText et = new EditText(mParent);
		et.setText(f.title);
		new AlertDialog.Builder(mParent)
				...
				.setNegativeButton(android.R.string.yes,
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
								...
								if (StringUtils.isEmpty(name) || name.equals(f.title))
									return;

								try {
									File fromFile = new File(f.path);
									File nf = new File(fromFile.getParent(), name);
									if (nf.exists()) {
										Toast.makeText(mParent, R.string.file_rename_exists, Toast.LENGTH_LONG).show();
									...
									Toast.makeText(mParent, R.string.file_rename_failed, Toast.LENGTH_LONG).show();
								}
							}
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		Intent intent = new Intent(mParent, VideoPlayerActivity.class);
	...
	private class DataTask extends AsyncTask<Void, Void, List<POMedia>> {
		...
		protected void onPostExecute(List<POMedia> result) {
			...
			mAdapter = new FileAdapter(mParent, result);
